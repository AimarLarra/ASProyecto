<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") // Liburu bat sartzen saiatzen badira..
    {
        // Ikusiko dugu lehenik ea liburu hori datubasean badagoen ala ez.
        $titulo = $_POST["titulo"];
        $texto = $_POST["texto"];

        $sql = "INSERT INTO posts (nombre, titulo, texto) VALUES ('$nombre','$titulo','$texto')";
        if (mysqli_query($con,$sql))
        {
            echo '<div class="error-message">Subido exitosamente!</div>';
        }
        else{
            echo '<div class="error-message">Ha habido un error!</div>';
        }
    ...
    <div class="container-fluid">
        <main class="tm-main">
            ...
            <div class="row tm-row tm-mb-120">
                <div class="col-12">
                    <h2 class="tm-color-primary tm-post-title tm-mb-60">Crea un post, <?php echo $nombre; ?>.</h2>
                </div>
                <div class="col-lg-7 tm-contact-left">
                    <form method="POST" action="makepost.php" class="mb-5 ml-auto mr-0 tm-contact-form">                        
                        <div class="form-group row mb-4">
                            <label for="subject" class="col-sm-3 col-form-label text-right tm-color-primary">Titulo</label>
                            <div class="col-sm-9">
                                <input class="form-control mr-0 ml-auto" name="titulo" id="titulo" type="text" required>
                            </div>
                        </div>
                        <div class="form-group row mb-5">
                            <label for="message" class="col-sm-3 col-form-label text-right tm-color-primary">Texto</label>
                            <div class="col-sm-9">
                                <textarea class="form-control mr-0 ml-auto" name="texto" id="texto" rows="8" required></textarea>                                
                            </div>
                        </div>
                        <div class="form-group row text-right">
                            <div class="col-12">
                                <button class="tm-btn tm-btn-primary tm-btn-small">Subir</button>                        
                            </div>                            
                        </div>                                
                    </form>
                </div>
                <div class="col-lg-5 tm-contact-right">
                    <address class="mb-4 tm-color-gray">
                        Escribe el tema y texto de tu post en los cuadros de texto
                    </address>
                    <span class="d-block">
                        Tel:
